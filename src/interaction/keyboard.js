	...
	this.create = function(){
		_keyBord = new _keyClass({
			display: {
				...
				'{EN}': '&nbsp;',
				'{abc}': '&nbsp;',
                '{rus}': i18next.t("interaction.keyboard.rus",'русский'),
                '{eng}': 'english',
                '{search}': i18next.t("interaction.keyboard.search",'найти'),
    ...
    this.speechRecognition = function(){
        ...
        if(SpeechRecognition){
			...
			recognition.addEventListener("start", ()=>{
                ...
                Noty.show( i18next.t("interaction.keyboard.noty_1",'Говорите, я слушаю..'))
            })
			...
			recognition.addEventListener("error", (event)=>{
                ...
                if (event.error == 'not-allowed') {
                    Noty.show(i18next.t("interaction.keyboard.noty_2",'Нет доступа к микрофону'))
                }
